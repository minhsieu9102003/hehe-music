import React, { useState, useEffect, useRef } from "react";
import Sidebar from "./Sidebar";
import { useWeather, weatherInfo } from "./WeatherContext";

function windDirection(deg) {
    const dirs = ["北", "北北東", "北東", "東北東", "東", "東南東", "南東", "南南東", "南", "南南西", "南西", "西南西", "西", "西北西", "北西", "北北西"];
    return dirs[Math.round(deg / 22.5) % 16];
}
function fmtH(t) { return t ? t.split("T")[1]?.slice(0, 5) : ""; }
function fmtDate(d) { const dt = new Date(d); return `${dt.getMonth() + 1}/${dt.getDate()}`; }
function dayName(d) { return ["日", "月", "火", "水", "木", "金", "土"][new Date(d).getDay()]; }
function isToday(d) { return new Date(d).toDateString() === new Date().toDateString(); }

function StatCard({ icon, label, value, unit, sub }) {
    return (
        <div className="wt-stat">
            <div className="wt-stat__icon">{icon}</div>
            <div className="wt-stat__info">
                <span className="wt-stat__label">{label}</span>
                <span className="wt-stat__value">{value}<small>{unit}</small></span>
                {sub && <span className="wt-stat__sub">{sub}</span>}
            </div>
        </div>
    );
}

export default function WeatherPage() {
    const { location, setLocation, current, hourly, daily, loading, error, searchCity, refresh, setMiniOpen } = useWeather();
    const [query, setQuery] = useState("");
    const [results, setResults] = useState([]);
    const [searching, setSearching] = useState(false);
    const searchRef = useRef(null);
    const debounceRef = useRef(null);

    useEffect(() => {
        clearTimeout(debounceRef.current);
        if (query.length < 2) { setResults([]); return; }
        debounceRef.current = setTimeout(async () => {
            setSearching(true);
            const r = await searchCity(query);
            setResults(r);
            setSearching(false);
        }, 400);
    }, [query, searchCity]);

    const selectCity = (c) => { setLocation(c); setQuery(""); setResults([]); };

    // Find today's hourly data
    const todayStr = new Date().toISOString().slice(0, 10);
    const todayHours = hourly.filter(h => h.time.startsWith(todayStr));
    const nowHour = new Date().getHours();

    const wi = current ? weatherInfo(current.weatherCode) : { icon: "🌡️", label: "" };

    if (loading && !current) return (
        <div className="hp-lay"><Sidebar activeId="" /><div className="hp-main"><div className="wt-loading">読み込み中...</div></div></div>
    );

    return (
        <>
            <style>{CSS}</style>
            <div className="hp-lay">
                <Sidebar activeId="" />
                <div className="hp-main">
                    <div className="wt-page">
                        {/* Header */}
                        <div className="wt-header">
                            <div className="wt-header__left">
                                <span style={{ fontSize: 28 }}>🌤️</span>
                                <h1>天気予報</h1>
                            </div>
                            <button className="wt-btn wt-btn--ghost" onClick={() => { setMiniOpen(true); }} title="ミニウィンドウ">
                                <svg width={16} height={16} viewBox="0 0 24 24" fill="none" stroke="#666" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="4 14 10 14 10 20" /><polyline points="20 10 14 10 14 4" /><line x1="14" y1="10" x2="21" y2="3" /><line x1="3" y1="21" x2="10" y2="14" /></svg>
                                <span>ミニ表示</span>
                            </button>
                        </div>

                        {/* Location search */}
                        <div className="wt-loc-row" ref={searchRef}>
                            <div className="wt-loc-current">
                                <svg width={18} height={18} viewBox="0 0 24 24" fill="none" stroke="#007bc3" strokeWidth="2" strokeLinecap="round"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z" /><circle cx="12" cy="10" r="3" /></svg>
                                <span>{location.name}</span>
                            </div>
                            <div className="wt-search-wrap">
                                <input className="wt-search" type="text" value={query} onChange={e => setQuery(e.target.value)} placeholder="都市名を検索..." />
                                {results.length > 0 && (
                                    <div className="wt-search-dropdown">
                                        {results.map((c, i) => (
                                            <div key={i} className="wt-search-item" onClick={() => selectCity(c)}>{c.name}</div>
                                        ))}
                                    </div>
                                )}
                            </div>
                            <button className="wt-btn wt-btn--icon" onClick={refresh} title="更新">🔄</button>
                        </div>

                        {error && <div className="wt-error">⚠️ {error}</div>}

                        {current && (
                            <>
                                {/* Current weather hero */}
                                <div className="wt-hero">
                                    <div className="wt-hero__main">
                                        <span className="wt-hero__icon">{wi.icon}</span>
                                        <div className="wt-hero__temp">
                                            <span className="wt-hero__deg">{Math.round(current.temp)}</span>
                                            <span className="wt-hero__unit">°C</span>
                                        </div>
                                    </div>
                                    <div className="wt-hero__details">
                                        <span className="wt-hero__label">{wi.label}</span>
                                        <span className="wt-hero__feels">体感 {Math.round(current.feelsLike)}°C</span>
                                    </div>
                                </div>

                                {/* Stats grid */}
                                <div className="wt-stats">
                                    <StatCard icon="💧" label="湿度" value={current.humidity} unit="%" />
                                    <StatCard icon="💨" label="風速" value={current.windSpeed} unit="km/h" sub={windDirection(current.windDir)} />
                                    <StatCard icon="🌧️" label="降水量" value={current.precipitation} unit="mm" />
                                    <StatCard icon="🔵" label="気圧" value={Math.round(current.pressure)} unit="hPa" />
                                    <StatCard icon="☁️" label="雲量" value={current.cloudCover} unit="%" />
                                    <StatCard icon="☀️" label="UV指数" value={current.uvIndex} unit="" sub={current.uvIndex >= 8 ? "非常に強い" : current.uvIndex >= 6 ? "強い" : current.uvIndex >= 3 ? "中程度" : "弱い"} />
                                </div>

                                {/* Today's sunrise/sunset */}
                                {daily.length > 0 && (() => {
                                    const today = daily.find(d => isToday(d.date)) || daily[3]; // past_days=3 so index 3 is today
                                    if (!today) return null;
                                    return (
                                        <div className="wt-sun-row">
                                            <div className="wt-sun">🌅 日の出 <b>{fmtH(today.sunrise)}</b></div>
                                            <div className="wt-sun">🌇 日の入 <b>{fmtH(today.sunset)}</b></div>
                                        </div>
                                    );
                                })()}

                                {/* Hourly forecast */}
                                <h2 className="wt-section-title">時間別予報（今日）</h2>
                                <div className="wt-hourly-scroll">
                                    <div className="wt-hourly">
                                        {todayHours.map((h, i) => {
                                            const hr = parseInt(h.time.split("T")[1]);
                                            const isCurrent = hr === nowHour;
                                            const hi = weatherInfo(h.code);
                                            return (
                                                <div key={i} className={`wt-hour ${isCurrent ? "is-now" : ""} ${hr < nowHour ? "is-past" : ""}`}>
                                                    <span className="wt-hour__time">{isCurrent ? "今" : `${hr}時`}</span>
                                                    <span className="wt-hour__icon">{hi.icon}</span>
                                                    <span className="wt-hour__temp">{Math.round(h.temp)}°</span>
                                                    <span className="wt-hour__rain">{h.rainProb}%</span>
                                                </div>
                                            );
                                        })}
                                    </div>
                                </div>

                                {/* Daily forecast (past + future) */}
                                <h2 className="wt-section-title">日別予報</h2>
                                <div className="wt-daily">
                                    {daily.map((d, i) => {
                                        const di = weatherInfo(d.code);
                                        const today = isToday(d.date);
                                        const isPast = new Date(d.date) < new Date(todayStr);
                                        return (
                                            <div key={i} className={`wt-day ${today ? "is-today" : ""} ${isPast ? "is-past" : ""}`}>
                                                <div className="wt-day__date">
                                                    <span>{fmtDate(d.date)}</span>
                                                    <span className="wt-day__dow">({dayName(d.date)})</span>
                                                    {today && <span className="wt-day__badge">今日</span>}
                                                    {isPast && <span className="wt-day__badge wt-day__badge--past">過去</span>}
                                                </div>
                                                <span className="wt-day__icon">{di.icon}</span>
                                                <span className="wt-day__label">{di.label}</span>
                                                <div className="wt-day__temps">
                                                    <span className="wt-day__max">{Math.round(d.maxTemp)}°</span>
                                                    <span className="wt-day__min">{Math.round(d.minTemp)}°</span>
                                                </div>
                                                <div className="wt-day__extra">
                                                    <span>💧{d.rainProb}%</span>
                                                    <span>🌧️{d.rain}mm</span>
                                                    <span>💨{d.wind}km/h</span>
                                                </div>
                                            </div>
                                        );
                                    })}
                                </div>
                            </>
                        )}
                    </div>
                </div>
            </div>
        </>
    );
}

const CSS = `
.hp-lay{display:flex;min-height:calc(100vh - 64px)}
.hp-main{flex:1;background:#fff;min-width:0;overflow-y:auto}
.wt-page{max-width:900px;margin:0 auto;padding:32px 48px 64px}
@media(max-width:1023px){.wt-page{padding:24px 24px 48px}}
@media(max-width:767px){.wt-page{padding:16px 16px 32px}}
.wt-loading{display:flex;align-items:center;justify-content:center;height:60vh;color:#999;font-size:16px}
.wt-error{background:#fce4ec;color:#c62828;padding:12px 16px;border-radius:8px;font-size:13px;margin-bottom:16px}

.wt-header{display:flex;align-items:center;justify-content:space-between;margin-bottom:16px}
.wt-header__left{display:flex;align-items:center;gap:10px}
.wt-header__left h1{font-size:20px;font-weight:700;color:#333;margin:0}

/* Location */
.wt-loc-row{display:flex;align-items:center;gap:12px;margin-bottom:24px;flex-wrap:wrap}
.wt-loc-current{display:flex;align-items:center;gap:6px;font-size:15px;font-weight:600;color:#333}
.wt-search-wrap{position:relative;flex:1;max-width:280px}
.wt-search{width:100%;height:36px;border:1px solid #dcdcdc;border-radius:8px;padding:0 12px;font-size:13px;font-family:inherit;color:#333;outline:none;transition:border-color .2s}
.wt-search:focus{border-color:#007bc3;box-shadow:0 0 0 2px rgba(0,123,195,.12)}
.wt-search::placeholder{color:#bbb}
.wt-search-dropdown{position:absolute;top:40px;left:0;right:0;background:#fff;border-radius:8px;box-shadow:0 4px 20px rgba(0,0,0,.12);z-index:100;max-height:240px;overflow-y:auto}
.wt-search-item{padding:10px 14px;font-size:13px;cursor:pointer;transition:background .15s}
.wt-search-item:hover{background:#f5f7fa}

/* Hero */
.wt-hero{display:flex;flex-direction:column;align-items:center;padding:32px 0 24px;gap:8px}
.wt-hero__main{display:flex;align-items:center;gap:16px}
.wt-hero__icon{font-size:64px;line-height:1}
.wt-hero__temp{display:flex;align-items:flex-start}
.wt-hero__deg{font-size:56px;font-weight:200;color:#333;line-height:1}
.wt-hero__unit{font-size:24px;color:#999;margin-top:8px}
.wt-hero__details{display:flex;flex-direction:column;align-items:center;gap:2px}
.wt-hero__label{font-size:16px;color:#555;font-weight:500}
.wt-hero__feels{font-size:13px;color:#999}

/* Stats */
.wt-stats{display:grid;grid-template-columns:repeat(3,1fr);gap:12px;margin-bottom:24px}
@media(max-width:700px){.wt-stats{grid-template-columns:repeat(2,1fr)}}
.wt-stat{display:flex;align-items:center;gap:12px;padding:14px 16px;border:1px solid #e8e8e8;border-radius:10px;background:#fafbfc}
.wt-stat__icon{font-size:24px;flex-shrink:0}
.wt-stat__info{display:flex;flex-direction:column;min-width:0}
.wt-stat__label{font-size:11px;color:#999;font-weight:500}
.wt-stat__value{font-size:18px;font-weight:600;color:#333}
.wt-stat__value small{font-size:12px;color:#999;margin-left:2px;font-weight:400}
.wt-stat__sub{font-size:11px;color:#808CBA}

/* Sun */
.wt-sun-row{display:flex;gap:24px;justify-content:center;margin-bottom:28px}
.wt-sun{font-size:14px;color:#666;display:flex;align-items:center;gap:6px}
.wt-sun b{color:#333}

/* Section title */
.wt-section-title{font-size:16px;font-weight:700;color:#333;margin:0 0 16px}

/* Hourly */
.wt-hourly-scroll{overflow-x:auto;margin-bottom:28px;padding-bottom:4px}
.wt-hourly{display:flex;gap:4px;min-width:max-content}
.wt-hour{display:flex;flex-direction:column;align-items:center;gap:6px;padding:10px 12px;border-radius:10px;min-width:60px;transition:background .15s}
.wt-hour.is-now{background:#e7f1fd;font-weight:600}
.wt-hour.is-past{opacity:.5}
.wt-hour__time{font-size:12px;color:#666}
.wt-hour__icon{font-size:20px}
.wt-hour__temp{font-size:15px;font-weight:600;color:#333}
.wt-hour__rain{font-size:10px;color:#007bc3}

/* Daily */
.wt-daily{display:flex;flex-direction:column;gap:4px}
.wt-day{display:flex;align-items:center;padding:12px 16px;border-radius:10px;gap:12px;transition:background .15s}
.wt-day:hover{background:#f5f7fa}
.wt-day.is-today{background:#e7f1fd}
.wt-day.is-past{opacity:.65}
.wt-day__date{display:flex;align-items:center;gap:4px;width:120px;flex-shrink:0;font-size:14px;color:#333;font-weight:500}
.wt-day__dow{font-size:12px;color:#999}
.wt-day__badge{font-size:10px;background:#007bc3;color:#fff;padding:1px 6px;border-radius:4px;margin-left:4px}
.wt-day__badge--past{background:#bbb}
.wt-day__icon{font-size:22px;flex-shrink:0}
.wt-day__label{font-size:13px;color:#666;width:100px;flex-shrink:0}
.wt-day__temps{display:flex;gap:8px;width:80px;flex-shrink:0}
.wt-day__max{font-size:14px;font-weight:600;color:#e53935}
.wt-day__min{font-size:14px;color:#007bc3}
.wt-day__extra{display:flex;gap:10px;font-size:11px;color:#999}
@media(max-width:700px){.wt-day{flex-wrap:wrap;gap:8px}.wt-day__date{width:auto}.wt-day__label{width:auto}}

/* Buttons */
.wt-btn{display:inline-flex;align-items:center;gap:6px;border:none;border-radius:8px;padding:8px 14px;font-size:13px;font-weight:500;cursor:pointer;font-family:inherit;transition:background .15s}
.wt-btn--ghost{background:#f0f2f5;color:#333}
.wt-btn--ghost:hover{background:#e4e6ea}
.wt-btn--icon{background:none;padding:6px;border-radius:50%;font-size:18px;width:36px;height:36px;display:flex;align-items:center;justify-content:center}
.wt-btn--icon:hover{background:#f0f2f5}
`;
